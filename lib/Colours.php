...
/**
 * A class to output terminal colour codes.
 *
 * Currently only supports the basic 16 colour mode.
 */
class Colours
{
  const E = "\e[";
  ...
  const T_FG = 3; // Foreground colour prefix
  const T_BG = 4; // Background colour prefix
  ...
  // And some predefined colours for backwards compatibility
  ...
  /**
   * A quick way to get a colour code.
   *
   * @param mixed $opts  Either an array of options, or null.
   *
   *  'fg'        (string)  The foreground colour.
   ...
   *  'reverse'   (bool)    Reverse the colours.
   *
   * If it's null, we reset back to normal colours.
   *
   * The accepted colour strings are:
   *
   * 'black'
   * 'red'
   * 'green'
  ...
  /**
   * A shortcut to setting the foreground colour.
   *
   * @param string $color  The colour for the foreground.
  ...
  /**
   * A shortcut to setting the background colour.
   *
   * @param string $color  The colour for the background.
   * @param bool $bold  (Optional, default false) Should the text be bold?
   * @param array $opts (Optional) Additional options for get().
   */
   ...
   *
   * @param string $colorname  The name of the colour.
   * @param bool $bg  (Optional, default false) Is it a background colour?
   * @return string  The two digit colour code.
   */
